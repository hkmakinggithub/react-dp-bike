...
useEffect(() => {
  const getNextNo = async () => {
    const res = await fetch('http://localhost:5000/api/cust-inward-no', {
  ...
  const fetchData = async () => {
      try {
        ...
        const numRes = await fetch('http://localhost:5000/api/cust-inward-no', {
          headers: { 'branch-id': activeBranch }
        });
        ...
        const custRes = await fetch('http://localhost:5000/api/customers-list', {
          headers: { 'branch-id': activeBranch }
        });
        const basicCustomers = await custRes.json();

        // 3. 🚨 THE FIX: Fetch Sales Data (to get Model & Chassis No for existing buyers)
        const salesRes = await fetch('http://localhost:5000/api/sales/list', {
          headers: { 'branch-id': activeBranch }
        });
        ...
        // 4. Get Parts List
        const partRes = await fetch('http://localhost:5000/api/parts', {
          headers: { 'branch-id': activeBranch }
        });
  ...
  try {
    ...
    const handleQuickAdd = async (type) => {
      ...
      const url = type === 'CUSTOMER' 
        ...
        : 'http://localhost:5000/api/add-part';
